...
def check_projects_count(data: Dict[str, Any], readme_path: Path) -> List[str]:
    ...
    if total_projects != stats_total:
        issues.append(f"⚠️  Incohérence : {total_projects} projets dans la liste mais {stats_total} dans les stats")
    
    # Vérifie dans le README
    if readme_path.exists():
        content = readme_path.read_text(encoding="utf-8", errors="ignore")
        # Cherche "12 projets" ou similaire
        if f"{total_projects} projets" not in content and f"{total_projects} en production" not in content:

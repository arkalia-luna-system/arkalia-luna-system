...
Usage:
    python -m github_profile.core.auto_update [--dry-run]
    ou après installation: auto-update [--dry-run]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="🌙 Mise à jour automatique du README principal"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--add-markers",
        action="store_true",
        help="Ajoute les marqueurs de sections si absents"
    )
    parser.add_argument(
        "--readme",
        type=str,
    ...
    if not data_file.exists():
        ...
        print("   Ou utilisez : ./scripts/update-all.sh")
        return 1
    ...
    content, stats_updated = update_readme_section(
        content, "stats", stats_content, args.dry_run
    )
    if stats_updated:
        updated = True

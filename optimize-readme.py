...
class ReadmeOptimizer:
    ...
    def analyze_duplicates(self) -> List[Dict]:
        ...
        if len(hero_matches) > 1:
            ...
            duplicates.append({
                "type": "hero_section_dupliquée",
                "count": len(hero_matches),
                "lines": lines
            })
...
"""
        ...
        for project in projects[:11]:  # Limite à 11
            ...
            table_lines.append(
                f"| {emoji} | **[{name}]({url})** | {desc}... | {lang} | {status} |"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="🔍 Analyseur et Optimiseur de README"
    )
    ...
    if analysis['duplicates']:
        ...
        for dup in analysis['duplicates']:
            print(f"   - {dup['type']} : {dup.get('count', 'N/A')} occurrences (lignes {dup.get('lines', [])[:3]})")
